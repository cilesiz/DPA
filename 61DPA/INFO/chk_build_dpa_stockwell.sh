...
then
echo "Successfully mount CIFS //uncle.wysdm.lab.emc.com/build/dpa/6.0.1/server to $tmpdpa"
echo
else
...
ls -lrt "$tmpdpa"/*/DPA-Server*Linux-x86_64*[0-9].bin | awk '{print NR"\t"$NF"\t\t""["$(NF-3)$(NF-2)"_"$(NF-1)"]"}' | awk -F"/" '{print $1" "$NF}'
ls -lrt "$tmpdpa"/*/DPA-Server*Linux-x86_64*[0-9].bin | awk -F"/" '{print "#"NR"#"$NF"#"}' > "$tmp1"
