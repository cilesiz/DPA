...
echo "#Set Password fro defaults Users#2#af5668c5-733d-4caa-b82e-193efada65a2#" >> "$tmpinfo"
...
echo
echo -n "Choose SETUPGUID ITEM to CHANGE [ No / Fullname / ID ] : "
read -e stpgd
...
echo "=================================================="
echo
cat "$tmp1" | awk "/<setupGuideGroup/,/<setupGuideItems>/" > "$tmpxml"
cat "$tmp1" | awk "/<setupGuideItem/,/setupGuideItem>/" | egrep "<id>|<ticked>" | awk -F"<" '{print $1,$2}' | awk -F">" '{print $1"="$2}' | paste - - | awk '{print "#"$1"#"$2"#"}' > "$tmp2"

i=1
for susun in `cat "$tmp2" | awk -F"#" '{print $2}' | awk -F"=" '{print $2}'`
do
tick_status=`cat "$tmp2" | grep "$susun" | awk -F"#" '{print $3}' | awk -F"=" '{print $2}'`
...
barisnilai=`cat -n "$tmp1" | grep "$susun" | awk '{print $1}'`
barisguna=$(($barisnilai - 1))

cat "$tmp1" | sed -n "$barisguna"p >> "$tmpxml"
...
curl -k -v -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X PUT -T "$tmpxml" "$URL_dpa_stupgd"/"$menu_id"
echo
echo
