...
listing_jer "$URL_dpa_viewlt" /viewletTemplates/viewletTemplate/id viewletTemplate "$tmp1" id name system description 
...
curl -k -v -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X GET "$URL_dpa_viewlt"/"$user_id" | xml fo
echo
