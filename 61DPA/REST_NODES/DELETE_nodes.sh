...
xml sel -t -m nodes/node -v numField -o "#" -v id -v numField -o "#" -v globalName -v numField -o "#" -v displayName -v numField -o "#" -n "$tmp1" | grep "#" | grep -v ":" | sort -t "#" -k 3 | awk -F"#" '{print "#"NR$0}' > "$tmp2"

echo; echo

cat "$tmp2" | awk -F"#" '{print $2"\t"$3"\t"$5}'

echo
echo -n "Choose NODE to DELETE [No/Node ID/Display Name] : "
read -e agentnak
...
curl -k -v -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X DELETE "$URL_apollo_nod"/"$node_id"
echo
echo
