...
echo; echo
echo "--> ALERT $agent_id NOTES"
echo; echo
curl -k -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X GET "$URL_dpa_alerts"/"$agent_id"/notes > "$tmp1"
echo; echo
xml fo "$tmp1"

echo; echo
echo "--> ALERT $agent_id ATTACHEDREPORTS"
echo; echo
curl -k -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X GET "$URL_dpa_alerts"/"$agent_id"/attachedReports > "$tmp1" 
echo; echo
xml fo "$tmp1"
