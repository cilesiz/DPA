...
listing_jer "$URL_dpa_chgbck" /chargebackPolicies/chargebackPolicy/id chargebackPolicy "$tmp1" id name enabled description 

echo
echo -n "Choose chargebackPolicy [ No / ID ] : "
read -e user_id_nk
...
curl -k -v -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X GET "$URL_dpa_chgbck"/"$user_id" | xml fo
echo
