...
mkdir "$tmpdpa"
mount -t cifs //uncle.wysdm.lab.emc.com/build/dpa/6.1.0/server "$tmpdpa" -o user=WYSDM/talibr,pass=irix293
echo
echo "SUCCESSFULLY MOUNT DPA's 6.1.0 trunk/server directory"
echo
...
ls -lrt "$tmpdpa"/*/DPA-Server*Linux*[0-9].bin | grep -v Patch | awk '{print NR"\t"$NF"\t\t""["$(NF-3)$(NF-2)"_"$(NF-1)"]"}' | awk -F"/" '{print $1" "$(NF-1)"/"$NF}'
ls -lrt "$tmpdpa"/*/DPA-Server*Linux*[0-9].bin | grep -v Patch | awk -F"/" '{print "#"NR"#"$NF"#"}' > "$tmp1"
...
mkdir "$tmp4" && \
unzip "$filechk1" -d "$tmp4" \
'$SERVER_SVN_ROOT$'/installer-layout/target/installer/common/apps/services/applications/apollo.ear \
'$SERVER_SVN_ROOT$'/installer-layout/target/installer/common/apps/services/applications/dpa.ear \
'$SERVER_SVN_ROOT$'/installer-layout/target/installer/common/apps/services/applications/ui.war
mkdir "$tmp2" && \
...
buildver1=`echo $filechk1 | awk -F"/" '{print $(NF-1)"/"$NF}'`
...
echo
echo "FOR REPORT PURPOSE -- DPA server $buildful1 (Apollo#$apollo_ver, DPA#$dpa_ver, UI#$ui_ver)"
echo
