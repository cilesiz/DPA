...
listing_jer_extra_agent "$URL_dpa_agents" /agentSettingsList/agentSettings/id agentSettings "$tmp1" name id agentId agentVersion status
...
curl -k -v -u "$login1":"$paswd1" \
-H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" \
-X GET "$URL_dpa_agents"/"$agent_id"/settings | xml fo
echo
