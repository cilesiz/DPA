#!/bin/bash

HB_DPA=10.64.205.162
user1=administrator
pswd1=administrator

URL1=http://"$HB_DPA":9004/dpa-api/agents

echo

curl -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1" > tmp1.txt

echo
echo
echo "LIST OF ALL AGENTS BEFORE DELETE"
echo

cat tmp1.txt | egrep -i "<name>|<id>" | awk -F">" '{print $2}' | awk -F"<" '{print $1}' | paste - - | cat -n
cat tmp1.txt | egrep -i "<name>|<id>" | awk -F">" '{print $2}' | awk -F"<" '{print $1}' | paste - - | cat -n | awk '{print ":"$1":"$2":"$NF":"}' > tmp2.txt
echo
echo -n "Choose agent to delete [No/Hostname/id] : "
read -e agentnak

cat tmp2.txt | grep -i ":$agentnak:" > tmp3.txt

berapa=`cat tmp3.txt | awk -F: '{print $2}'`
siapanya=`cat tmp3.txt | awk -F: '{print $3}'`
system_id=`cat tmp3.txt | awk -F: '{print $4}'`

echo
echo "Number = $berapa"
echo "Hostname = $siapanya"
echo "ID = $system_id"
echo

if [ $berapa -gt 0 2> /dev/null ]
then

echo
echo "START TO DELETE AGENT $siapanya"
echo "================================"
echo
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X DELETE "$URL1"/"$system_id"/settings
echo
echo "SUCCESSFULLY DELETE AGENT $siapanya"
echo "====================================="
echo

curl -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1" > tmp4.txt

echo
echo
echo "LIST OF ALL AGENTS AFTER DELETE"
echo
cat tmp4.txt | egrep -i "<name>|<id>" | awk -F">" '{print $2}' | awk -F"<" '{print $1}' | paste - - | cat -n
echo
rm -rf tmp1.txt tmp2.txt tmp3.txt tmp4.txt

else

echo "WRONG INPUT"
echo

fi
