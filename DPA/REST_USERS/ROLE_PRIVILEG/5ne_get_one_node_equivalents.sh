...
URL1=http://"$HB_DPA":9004/dpa-api/nodes
...
echo
echo -n "Choose Node to check it's Equivalents [No/Node Name/Node ID] : "
read -e agentnak
...
echo
# curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1"/"$node_id"/equivalents
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL2"/"$node_id"/equivalents
echo
echo
