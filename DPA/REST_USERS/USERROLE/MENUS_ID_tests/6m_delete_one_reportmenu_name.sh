...
echo
echo
echo "CHOOSE REPORTMENU TO DELETE"
...
echo
echo "No = $berapa / ReportMenu = $role1 / ID = $role1_id"
echo
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X DELETE "$URL1"/"$role1_id"
echo
