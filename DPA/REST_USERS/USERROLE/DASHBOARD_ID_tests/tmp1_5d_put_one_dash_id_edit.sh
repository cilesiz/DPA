...
nombor=`cat tmp2.txt | grep "#$agentnak#" | awk -F"#" '{print $2}'`
namanya=`cat tmp2.txt | grep "#$agentnak#" | awk -F"#" '{print $3}'`
...
echo
echo "EXISTING CONFIGURATION FOR DASHBOARD = $namanya"
echo "=================================================="
echo
echo "VIEWLETS"
i_v=1
...
nombor=`cat tmp1v2.txt | grep "#$viewltnak#" | awk -F"#" '{print $2}'`
namanya=`cat tmp1v2.txt | grep "#$viewltnak#" | awk -F"#" '{print $(NF-1)}'`
viewlet_id=`cat tmp1v2.txt | grep "#$viewltnak#" | awk -F"#" '{print $3}'`

echo
echo "CHOOSEN : No = $nombor / Name = $namanya / ID = $viewlet_id"
echo
...
cat tmp1.xml | awk "/<authorization>/,/<\/read>/" | grep -v "</read>" | grep -v "</userRoles>" | grep -v "<userRoles/>" >> tmp12.xml
...
read -e userrole1
nombor=`cat tmp1r2.txt | grep -i "#$userrole1#" | awk -F"#" '{print $2}'`
...
echo
echo "CHOOSEN : No = $nombor / Userrole = $role1 / Usrrole_ID = $role1_id "
echo
...
cat tmp1.xml | awk "/<readWrite>/,/<\/readWrite>/" | grep -v "</readWrite>" | grep -v "</userRoles>" | grep -v "<userRoles/>" >> tmp12.xml
...
echo
echo "CHOOSEN : No = $nombor / Userrole = $role1 / ID = $role1_id "
echo
echo "            <userRole> "  >> tmp12.xml
echo "               <id>$role1_id</id> "  >> tmp12.xml
echo "            </userRole> "  >> tmp12.xml
