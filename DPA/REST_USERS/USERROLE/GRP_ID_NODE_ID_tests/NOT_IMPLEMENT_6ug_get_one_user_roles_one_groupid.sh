...
echo
echo
echo "To GET INFO One Groups list for given UserRole"
echo
echo "CHOOSE ROLE"
...
echo "CHOOSE GROUPP_ID TO GET INFO"
...
echo
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1"/"$role1_id"/groups/"$group_id" > tmp3.txt
echo
