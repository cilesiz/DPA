...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X POST -d '
   <userRole version="1">
      <name>ApolloUser_uis_0x0</name>
      <description>UserRole for a Apollo User - U I S - 0x0</description>
      <permissions>
...
sed -i "s/idtukar=.*/idtukar=$tukar/g" POST_create_user_au_uis_0x0.sh 
rm -rf tmp1.txt 
