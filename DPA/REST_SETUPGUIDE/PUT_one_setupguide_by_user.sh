...
#################################### HARDCODED VALUE SETUP BY DEVELOPER #########################
echo "#Enter DPA License Key#1#c6fea272-a2fb-4dcb-aa57-2c55e4ecd4ec#" > "$tmpinfo1"
echo "#Change Password#2#af5668c5-733d-4caa-b82e-193efada65a2#" >> "$tmpinfo1"
echo "#Discover The Environment#3#8ddbd837-0b8d-4c91-90de-037b7801cb0b#" >> "$tmpinfo1"
echo "#Configure Users & Security#4#f48f9245-ebed-4424-ab13-4eb96d97b7a1#" >> "$tmpinfo1"
echo "#Create Groups of objects#5#e15882fb-2eb6-4082-a501-17fc31c1b45a#" >> "$tmpinfo1"
echo "#Configure Alerting#6#ac129e7b-a870-427a-8b63-38dcac162a9a#" >> "$tmpinfo1"
echo "#How to Customise Dashboards#7#8aeeab31-36b7-456a-85e3-1db0c4cb0f96#" >> "$tmpinfo1"
###########################################################################################
...
do
echo -n "$i"
cat "$tmpinfo1" | grep "#$grp_id#" | awk -F"#" '{print " - "$2" - (ID) "$4}'
echo -n "#$i#" >> "$tmp4"
cat "$tmpinfo1" | grep "#$grp_id#" | awk -F"#" '{print $2"#"$4"#"}' >> "$tmp4"
i=$((i + 1))
done 
...
echo
echo
echo "EXISTING SETUPGUIDES - $namanya"
echo "============================================"
echo
cat "$tmp5"
echo
echo
echo "SETUP NEW VALUE TO EXISTING SETUPGUIDES - $namanya"
echo "=================================================="
echo
cat "$tmp5" | awk "/<setupGuideGroup/,/<setupGuideItems>/" > "$tmpxml"
cat "$tmp5" | awk "/<setupGuideItem/,/setupGuideItem>/" | egrep "<id>|<ticked>" | awk -F"<" '{print $1,$2}' | awk -F">" '{print $1"="$2}' | paste - - | awk '{print "#"$1"#"$2"#"}' > "$tmp6"

i=1
for susun in `cat "$tmp6" | awk -F"#" '{print $2}' | awk -F"=" '{print $2}'`
do
tick_status=`cat "$tmp6" | grep "$susun" | awk -F"#" '{print $3}' | awk -F"=" '{print $2}'`
echo -n "$i -> [ID]$susun [TICKED]$tick_status [FULLNAME]"
cat "$tmpinfo2" | grep "$susun" | awk -F"#" '{print $2}'
echo -n "        -> CURRENTLY >> TICKED = $tick_status -> NEW value to set [ true / false ] : "
read -e tick_new

while [ $tick_new != true -a $tick_new != false ]
do
echo -n  "            -> WRONG VALUE '$tick_new' -> PUT AGAIN  [ true / false ] : " 
read -e tick_new
done

barisnilai=`cat -n "$tmp5" | grep "$susun" | awk '{print $1}'`
barisguna=$(($barisnilai - 1))

cat "$tmp5" | sed -n "$barisguna"p >> "$tmpxml"
echo "         <id>$susun</id> " >> "$tmpxml"
echo "         <ticked>$tick_new</ticked> " >> "$tmpxml"
echo "      </setupGuideItem> " >> "$tmpxml"

i=$(( $i + 1))

done

echo "   </setupGuideItems> " >> "$tmpxml"
echo "</setupGuideGroup> " >> "$tmpxml"
echo
echo "INPUT XML"
echo "========="
echo
...
echo
echo
curl -v -u "$usernya":"$pswdnya" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X PUT -T "$tmpxml" "$URL1"/"$menu_id"
echo
echo
...
rm -rf "$tmpinfo1" "$tmpinfo2" "$tmp1" "$tmp2" "$tmp3" "$tmp4" "$tmp5" "$tmp6" "$tmpxml"
