...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X POST -d '
   <userRole version="1">
      <name>ReplicationAnalysis100</name>
      <description>UserRole for a Replication Analysis 100</description>
      <permissions>
...
sed -i "s/idtukar=.*/idtukar=$tukar/g" POST_create_user_ra_ro.sh 
rm -rf tmp1.txt 
