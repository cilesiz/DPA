...
echo
echo -n "Choose Alert AttachedReports to check [ No / ID ] : "
read -e agentnak
...
echo
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1"/"$system_id"/attachedReports
echo
echo
