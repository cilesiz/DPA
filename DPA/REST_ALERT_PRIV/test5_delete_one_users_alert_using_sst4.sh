...
echo
echo -n "Choose Alert to DELETE [ No / ID ] : "
read -e agentnak
...
echo
curl -v -u "$user2":"$pswd2" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X DELETE "$URL1"/"$system_id"
echo
echo
...
echo
echo "Alert after Delete $system_id"
echo

cat tmp1.txt | grep -i "dpa-api/alert" | awk -F"/" '{print $6}' | awk -F'"' '{print $1}' > tmp2a.txt
cat tmp1.txt | grep -i "<description>" | awk -F">" '{print $2}' | awk -F"<" '{print $1}' > tmp2b.txt
paste tmp2a.txt tmp2b.txt | cat -n
rm -rf tmp1.txt tmp2a.txt tmp2b.txt
echo
