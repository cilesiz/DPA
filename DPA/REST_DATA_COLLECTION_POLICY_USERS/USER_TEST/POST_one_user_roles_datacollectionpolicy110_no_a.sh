...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X POST -d '
   <userRole version="1">
      <name>DataCollectionPolicy110</name>
      <description>UserRole for a Data Collection Policy 110</description>
      <permissions>
...
sed -i "s/idtukar=.*/idtukar=$tukar/g" POST_create_user_dcp_noa_no_as.sh 
rm -rf tmp1.txt 
