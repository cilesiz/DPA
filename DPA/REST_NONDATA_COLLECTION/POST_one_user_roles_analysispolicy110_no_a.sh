...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X POST -d '
   <userRole version="1">
      <name>AnalysisPolicy110</name>
      <description>UserRole for a Analysis Policy 110</description>
      <permissions>
...
sed -i "s/idtukar=.*/idtukar=$tukar/g" POST_create_user_ap_noa_no_as.sh 
rm -rf tmp1.txt 
# -rwxr-xr-x 1 root root 923 Mar  8 11:40 POST_create_user_ap_noa_no_as.sh
# -rwxr-xr-x 1 root root 923 Mar  8 11:41 POST_create_user_ap_nor_no_ro.sh
#      <system>false</system>
