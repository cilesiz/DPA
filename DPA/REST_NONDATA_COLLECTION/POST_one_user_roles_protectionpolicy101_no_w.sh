...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X POST -d '
   <userRole version="1">
      <name>ProtectionPolicy101</name>
      <description>UserRole for a Protection Policy 101</description>
      <permissions>
...
sed -i "s/idtukar=.*/idtukar=$tukar/g" POST_create_user_pp_now_no_w.sh 
rm -rf tmp1.txt 
