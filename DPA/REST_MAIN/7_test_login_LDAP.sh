...
if [ $LDAP_IP == 10.64.213.7 -o $LDAP_IP == 10.64.213.8 ]
then
...
berapa=`cat "$tmp6" | awk -F# '{print $2}'`
siapanya=`cat "$tmp6" | awk -F# '{print $3}'`
...
echo
echo "LOGON TEST FOR USER ($user2) / PSSWD ($pswd2) TO GET USER $user2 INFO "
echo "====================================================================="
echo
echo
curl -v -u "$user2":"$pswd2" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL1"/"$user2"
echo
echo
