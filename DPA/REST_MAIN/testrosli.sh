...
echo
curl -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -X GET "$URL2" > tmp1fake.txt
echo

cat tmp1fake.txt | egrep -v "<id>|<link" | sed -e 's/<groupRole version.*/<groupRole version="1">/g' -e 's/<ldapSettings.*/<ldapSettings version="1">/g' > tmp2fake.xml
...
echo 
echo
sed -i "s/<allowAutoLogon>/\t TOREPLACELINE1XXXXX \n \t TOREPLACELINE2XXXXX  \n   <allowAutoLogon>/g" tmp2fake.xml
sed -i "s/TOREPLACELINE1XXXXX.*/$line1/g" tmp2fake.xml
sed -i "s/TOREPLACELINE2XXXXX.*/$line2/g" tmp2fake.xml
cat tmp2fake.xml
echo -n > tmp3fake.txt
...
curl -v -u "$user1":"$pswd1" -H "Accept: application/vnd.emc.apollo-v1+xml" -H "Content-Type: application/vnd.emc.apollo-v1+xml" -T tmp2fake.xml -X POST "$URL3" > tmp3fake.txt 2>&1
...
fakeuserid=`cat tmp3fake.txt | grep -i "user-test-request" | grep -i "result" | awk -F"/" '{print $7}'`
